...
approach is to make use of visual cues, which make areas of interactions
visible to the user by changing the cursor upon entering the cue area.
Visual cues are usually associated with changes to the resolution of a
representation or scales of a display. See Figure \ref{fig:hist} for an
example of a histogram with visual cues.
