...
\texttt{reactive programming} in the framework of the MVC architecture and
...
views. For example, we may examine the distribution of a variable from a
...
modify the data source as the user interacts with the plot, and as soon as the
...
own attributes, which are described by the meta data 
beyond the mutaframes. A meta data object
is also reactive, but it is only linked to a specific plot. For example, 
the bin-width of a histogram is stored in its meta data, and by the users
actions, the histogram provokes just to respond.
...
a series of projections into two dimensional space. By choosing projections, 
...
Interactivity of a mutaframe can be propagated to its subsets, which
allows multiple applications based on one mutaframe and its
...
to draw a new plot. The new plot is automatically connected with the original
plot: when we interact with the new plot, the selection will be passed to
the mutaframe and propagated to the original plot. This is similar to an example
...
approach is to make use of visual cues, which suggest available interactions
to the user, for example changing the cursor upon entering the cue area.
Visual cues are usually associated with changes to the resolution of a
representation or scales of a display. Figure \ref{fig:hist} shows an
example of a plot with visual cues.
...
{cranvas}, and both direct manipulation and indirect manipulation 
...
The other type of reactive data objects in \textbf{cranvas} are the meta data. 
...
meta data based on the mouse wheel event.
...
display of the corresponding values of the meta data. For the
