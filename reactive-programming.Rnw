...
Data Desk \citep{velleman1988} and LISP-STAT \citep{tierney2009} provided
tight integration with interactive graphics as well as numerical modeling.
In particular, LISP-STAT is also a programmable environment like R.

\hh{This needs quite a bit of extension, missing Maindonald and some of Oldford's  - we should also move some of the discussion from section 4 to here.}
...
and tight integration with modeling that R furnishes.
...
data. In the above example, the plot will be updated as soon as object
...
application in interactive graphics. \citet{shiny} is another application of
reactive programming in the R community, which makes it easy to interact
between HTML elements and R, but it does not have a specific emphasis on
...
the model. For an application of interactive graphics in the \texttt
...
{reactiveBrushed}, since the plot will be updated automatically. A real
...
to the view. Simultaneously, changes are made to a second mutaframe
containing information about the axis and labels.

\begin{figure}
\includegraphics[width=0.6\linewidth, height=0.6\linewidth]{figure/tour}

\caption{Grand tour of through the flea data. The data separate into three
clusters. Watch the video online at \url{http://cranvas.org/examples/qtour.html}.
\label{tour}}
\end{figure}
...
to draw a new plot. Then the new plot is naturally connected with the
original plot: when we interact with the new plot, the interaction will be
passed to the original plot, and vice versa.
