...
first described by \citet{Krasner1988} and invented by Trygve Reenskaug in
the 1970's.
% \hh{What are the architecture patterns used in early implementations of
% interactive statistical gaphics? Could you give them a name?}
It is widely used in GUI (Graphical User Interface) applications, including
web applications \citep{leff2001web}. The main reason for its popularity is
because it minimizes the dependencies between  different components of an
application.
