...
package, and its most important components, mutaframes and metadata
...
own attributes that are described by the metadata
beyond the mutaframes. A metadata object
is also reactive, but it is only linked to a specific plot. For example,
the bin-width of a histogram is stored in its metadata, and by the user's
