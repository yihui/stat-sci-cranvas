...
% ---
% I was assuming the audience familar should be very familiar with this (Yihui)
% ---
...
\hh{I wouldn't call what is done 'brushing' - to me, brushing has a graphical component that I am completely missing in this setting. What is shown is 'highlighting'. Could you try to re-phrase the example?}
...
controller(1:4)  # the first four point
...
\caption{\label{cartogram} Morph from a choropleth chart of the US (top left) to a population based cartogram (bottom right). The color represents electoral votes of states towards the Democratic (blue) or Republican party (red) in the 2012 Presidential election.}
\end{figure}
...
\bibliography{reactive-programming,R-packages,phd-thesis-yihui-xie,/part-i}
